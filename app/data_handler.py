...
def scrape_akshare(keywords, start_date, end_date, placeholder):
    """备用数据源：使用 AkShare，并从最近日期开始反向查询。"""
    all_results = []
    delta = end_date - start_date
    date_list = [start_date + timedelta(days=i) for i in range(delta.days + 1)]
    total_days = len(date_list)

    # 【关键修改】使用 reversed() 反转日期列表
    for i, single_date in enumerate(reversed(date_list)):
        date_str = single_date.strftime('%Y%m%d')
        
        # 实时更新状态
        progress_day_count = i + 1
        placeholder.info(f"⏳ 正在反向检索备用源: {single_date.strftime('%Y-%m-%d')} ({progress_day_count}/{total_days})... 已找到 {len(all_results)} 条公告。")
    ...
    final_df.sort_values(by='公告日期', ascending=False, inplace=True) # 最后仍然整体排序确保顺序
    return final_df
